...
const App = () => {
    ...
    const fetchShows=useCallback(async(searchTerm)=>{
  ...
  return (
   ...
   {popup && (<div id="backdrop" onClick={()=>setPopup(false)} className="z-[50] fixed left-0 right-0 top-0 bottom-0 backdrop-brightness-50 max-md:backdrop-brightness-50 !h-full !w-full" />)}
    {popup && (<div className='!fixed -translate-x-2/4 -translate-y-2/4 z-[200] shadow-2xl left-2/4 top-1/2 !w-[75%] max-lg:!w-[85%] max-md:!w-[90%]'>  <div className='w-full bg-slate-100 rounded-md relative animate-[pop_0.4s_ease-out]'>
       <header className='w-full md:pl-5 pl-2'>
        <div className="w-full  py-2 flex items-center gap-2"><div className=' text-3xl font-bold'>{details.name}</div></div>
       ...
       <div className='text-black px-2 max-md:flex-col select-none pr-4 max-md:px-3 pb-4 flex items-start gap-4 relative'>
        ...
        <div id='details' className='text-black drop-shadow-sm flex flex-col gap-3'>
          ...
          <div className='text-xl'><span>About:</span><div className='text-xl font-bold h-[16rem] overflow-y-scroll overflow-x-hidden'>{newTextSumm}</div></div>
        </div>
       </div>
    </div>
    </div>)}
   ...
   SHOW HUB
    ...
    <div id='search Actions' className='flex max-lg:flex-col max-lg:gap-8 lg:justify-between items-center w-[90%]'>
        {isLoading ?(
             <div className='w-[50%] max-lg:w-full max-lg:justify-center max-md:justify-normal [&>*]:bg-light-white [&>*]:animate-pulse [&>*]:p-1 [&>*]:px-2 [&>*]:rounded-md [&>*]:cursor-pointer flex gap-4 max-lg:[&>*]:text-[1rem] items-center max-md:overflow-x-scroll max-md:py-3 scroll-smooth max-md:rounded-3xl max-md:px-4 text-transparent'>
              {genres && genres.map((item,idx)=><span key={idx}>{item}</span>)}
              </div>
                
        ):(
        <div className='w-[50%] max-lg:w-full max-lg:justify-center max-md:justify-normal [&>*]:bg-light-white [&>*]:p-1 [&>*]:px-2 [&>*]:rounded-md [&>*]:cursor-pointer hover:[&>*]:bg-[rgba(108,108,108,0.25)] flex gap-4 max-lg:[&>*]:text-[1rem] items-center max-md:overflow-x-scroll max-md:py-3 scroll-smooth max-md:rounded-3xl max-md:px-4 select-none'>
        ...
        {isLoading?(
        <div id='search-skeleton' className='flex w-[35%] max-lg:w-[55%] max-md:w-full rounded-md bg-light-white animate-pulse'>
        ...
        <button className='p-2 px-6 text-transparent bg-transparent rounded-r-md'>Search</button>
        </div>

        ):(
            <div id='search-section' className='flex w-[35%] max-lg:w-[55%] max-md:w-full'>
            <div className='w-full max-md:w-full max-lg:w-full'>
